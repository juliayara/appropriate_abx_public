...
immunesuppressionphrases=c("immunosuppressed","immunocompromised","immune compromised", "immune suppressed")
...
criticalillness<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(criticalillnessphrases,collapse='|')))

#apply pleural effusion phrases
pleuraleffusion<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(pleuraleffusionphrases,collapse='|')))

#apply immune suppressed phrases
immunesuppressed<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(immunesuppressionphrases,collapse='|')))
...
#apply guidelines
maxabxchoice<-maxabxchoice %>%
  mutate(correctinitialabx= if_else(servicelocation=="Inpatient"&(correctinitialabx==2 |correctinitialabx==4)&(Med %in% optreatmentorunimmunized),1,correctinitialabx,correctinitialabx))
...
maxabxchoice %>%
  group_by(correctinitialabx,label)%>%
  summarise(count=n_distinct(CSN))%>%
  ungroup()%>%
  arrange(desc(count))
