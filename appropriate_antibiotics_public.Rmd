...
#negative treatment failure phrases
negtreatmentfailureprefix<-c("no","unlikely","less likely","did not", "has not", "didn't","hasn't", "has not yet","hasn't yet","did not yet", "didn't yet")
negtreatmentfailure<-levels(interaction(negtreatmentfailureprefix,treatmentfailurephrases,sep = ' '))

#aspiration phrases
aspirationphrases=c("aspiration pneumonia","likely aspiration", "aspiration event")
#negative aspiration phrases
negaspirationprefix=c("unlikely","less likely","is unlikely","is less likely","no","not likely","reassuring against","no concern for","am not concerned for","no history of","no recent episodes of")
negaspirationphrases1<-levels(interaction(aspirationphrases,negaspirationprefix,sep=' '))
negaspirationphrases2<-levels(interaction(negaspirationprefix,aspirationphrases,sep = ' '))
negaspaspirationphrasescomb<-c(negaspirationphrases1,negaspirationphrases2)

#immunization phrases
unimmunizedphrases=c("unimmunized","unvaccinated","partially vaccinated","partially immunized","not immunized","not fully immunized", "not up to date on immunizations", "immunizations not up to date", "immunizations are not up to date","not vaccinated")

#critical illness phrases
criticalillnessphrases=c("shock","critically ill","sepsis","critical illness","septic")
negcriticalillnessprefix=c("no","not","not in","no signs of","no sign of","not concerned for","no concern for","reassured against","reassuring against","no evidence of","not suggestive of")
negationcriticalillnessphrases=levels(interaction(negcriticalillnessprefix,criticalillnessphrases,sep = ' '))

#pleural effusion
pleuraleffusionphrases=c("moderate pleural effusion","large pleural effusion","moderate parapneumonic effusion","large parapneumonic effusion","loculated pleural effusion","complex pleural effusion")
negpleuraleffprefix=c("no","no signs of a","no sign of a","not concerned for a","no concern for a","reassured against a","reassuring against a","no evidence of a","not suggestive of a")
negplauraleff<-levels(interaction(negpleuraleffprefix,pleuraleffusionphrases,sep = ' '))

#immunocomrpomised
immunesuppressionphrases=c("immunosuppressed","immunocompromised","immune compromised", "immune suppressed","immunodeficiency","immunodeficient","immune deficiency","immune deficient")
negimmunosuppprefix<-c("no","not","no signs of","no sign of","not concerned for","no concern for","reassured against","reassuring against","no evidence of","not suggestive of")
negimmunosupp<-levels(interaction(negimmunosuppprefix,immunesuppressionphrases,sep = ' '))
...
treatmentfailure<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(treatmentfailurephrases,collapse='|'))&!str_detect(notedata$NoteText,paste(negtreatmentfailure,collapse = '|')))

#apply aspiration phrases
aspiration<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(aspirationphrases,collapse='|'))&!str_detect(notedata$NoteText,paste(negaspaspirationphrasescomb,collapse = '|')))
...
criticalillness<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(criticalillnessphrases,collapse='|'))&!str_detect(notedata$NoteText,paste(negationcriticalillnessphrases,collapse = '|')))

#apply pleural effusion phrases
pleuraleffusion<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(pleuraleffusionphrases,collapse='|'))&!str_detect(notedata$NoteText,paste(negplauraleff,collapse = '|')))

#apply immune suppressed phrases
immunesuppressed<-notedata %>%
  select(CSN) %>%
  filter(str_detect(notedata$NoteText, paste(immunesuppressionphrases,collapse='|'))&!str_detect(notedata$NoteText,paste(negimmunosupp,collapse = '|')))
