...
```{r}
abxculturedatatest<-abxculturedata %>%
  filter(abxculturedata$CSN==6122070797)
abxculturedatatest
```
...
```{r}
abxculturedatatest<-abxchoice %>%
  filter(CSN==6121563611)
...
unimmunizedphrases="(?i)unimmunized|unvaccinated|partially (vaccinated|immunized)|not (immunized|fully immunized|up to date on immunizations|vaccinated)|immunizations (not up to date|are not up to date)"
