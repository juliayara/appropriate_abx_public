...
library(caret)
#load confusion table for making confusion matrix into datatable

#install.packages("ConfusionTableR")
library(ConfusionTableR)

#installpsych package for stats calculations
...
cephalosporinallergy=c("cefdinir","cefotaxime","ceftriaxone","cephalexin","cephalosporins","cefuroxime","cefazolin","cefadroxil","cefotetan","cefoxitin","ceftazidime","cefpodoxime","cefixime","cefepime","ceftaroline","keflex") #list of meds that count as cephalosporin allergy
...
abxculturedata<-abxculturedata%>%
  mutate(reasonforexclusions= if_else(CSN=='6125650612' & reasonforexclusions=="Included","Excluded diagnosis",
                                      if_else(reasonforexclusions!="Included",reasonforexclusions,"Included"))) 
abxculturedata<-abxculturedata%>%
  mutate(reasonforexclusions= if_else(CSN=='6125701634' & reasonforexclusions=="Included","Excluded diagnosis",
                                      if_else(reasonforexclusions!="Included",reasonforexclusions,"Included"))) 
abxculturedata<-abxculturedata%>%
  mutate(reasonforexclusions= if_else(CSN=='6121967363' & reasonforexclusions=="Included","Excluded diagnosis",
                                      if_else(reasonforexclusions!="Included",reasonforexclusions,"Included"))) 
...
#write.csv(exclusions,"/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/ExclusionsVALIDATION.csv", row.names = FALSE) (#this doesn't update summary table below, FYI)
...
test<-"6125805272" %in% csnlist
test
#save CSN list as CSV

#write.csv(csnlist,"/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/CSNListValidation.csv", row.names = FALSE)

#for validation calcs only: get list of all correct & export to excel
#maxabxchoicecorrect<-maxabxchoice %>%
#  filter(label=="Correct")%>%
#  select(CSN)
#maxabxchoicecorrectcsn<-unique(maxabxchoicecorrect$CSN)

#write.csv(maxabxchoicecorrectcsn,"/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/CorrectListValidation2_20.csv", row.names = FALSE)
  
#for validation calcs only: get list of all exlcuded & export to excel
#exclusions<-abxculturedata %>%
#  filter(reasonforexclusions!="Included")
#exclusionsCSN<-unique(exclusions$CSN)
#write.csv(exclusionsCSN,"/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/ExclusionsVALIDATION2_20.csv", row.names = FALSE)
...
notefile="/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/NoteContentsValidation.csv"
...
```{r}
#upload validated csv doc (for statistics/summarization after your manual review)
stats<-read_csv("/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/NoteDataContextValidationIRR3_1.csv")
```
```{r}
#testing stuff
df1<-maxabxchoice%>%
...
#rename columns
stats <- stats %>%
  rename(Predicted.Class = labeldetailed) %>%
  rename(Actual.Class= actuallabel)
...
cm <- confusionMatrix(stats$Predicted.Class, stats$Actual.Class,mode = "everything")
...
```{r}
#summary stats without part 1 included
#Summary statistics

# Subset the data to exclude the "excluded" class, also exclude likely incorrect (redundant label)
stats_NLP<-stats%>%
  filter(!Predicted.Class=="Correct")

# Convert the columns to factors and specify the levels
stats_NLP$Predicted.Class <- factor(stats_NLP$Predicted.Class, levels = unique(stats_NLP$Predicted.Class))
stats_NLP$Actual.Class <- factor(stats_NLP$Actual.Class, levels = unique(stats_NLP$Actual.Class))

# Create the confusion matrix
cm <- confusionMatrix(stats_NLP$Predicted.Class, stats_NLP$Actual.Class,mode = "everything")
...
cmresults<-print(cm)
matrix<-as.matrix(cmresults, what = "classes", mode="everything")
matrix
statscount_actual<-stats%>%
  group_by(Actual.Class)%>%
  count()
...
ratings<-stats%>%select(firstrater,secondrater)
agree(ratings)
kappa2(ratings)
...
```{r}
#take reuploaded reviewed output and combine into new table (label, predicted category, actual category)
#make sankey
...
# Summarize the data by predicted class, actual class, and count
stats_summary <- stats %>% 
  group_by(Predicted.Class, Actual.Class) %>% 
  summarise(Count = n()) %>% 
  filter(!Predicted.Class=="Correct")

stats_summary$Actual.Class=as.character(paste("Actual_",stats_summary$Actual.Class))

links<-data.frame(stats_summary)

# From these flows we need to create a node data frame: it lists every entities involved in the flow

nodes <- data.frame(
  name = unique(c(factor(stats_summary$Predicted.Class), factor(stats_summary$Actual.Class)))
) %>% 
  arrange(name) %>% 
...
nodes <- nodes %>%
  mutate(group = ifelse(name %in% c("Likely incorrect abx","Actual_ Likely incorrect abx"), "Likely incorrect abx",
                        ifelse(name %in% c("Outpatient treatment failure","Actual_ Outpatient treatment failure"), "Outpatient treatment failure",
                               ifelse(name %in% c("Aspiration pneumonia","Actual_ Aspiration pneumonia"), "Aspiration pneumonia",
                                      ifelse(name %in% c("Large/Complicated Pleural Effusion","Actual_ Large/Complicated Pleural Effusion"), "Large/Complicated Pleural Effusion",
                                             ifelse(name %in% c("Critically ill","Actual_ Critically ill"), "Critically ill",
                                                    ifelse(name %in% c("Immunosuppressed","Actual_ Immunosuppressed"), "Immunosuppressed", "Other")
                                             )
                                      )
                               )
                        )
...
links$IDsource <- match(links$Predicted.Class, nodes$name)-1 
links$IDtarget <- match(links$Actual.Class, nodes$name)-1
 
# Make the Network
p <- sankeyNetwork(Links = links, Nodes = nodes,
              Source = "IDsource", Target = "IDtarget",
              Value = "Count", NodeID = "name", 
              sinksRight=FALSE, nodeWidth = 30,NodeGroup = "group",fontSize = 18)
p
...
#write.csv(trainreview,"/Volumes/dmcommon/_DM.Private/CI Program Faculty/+Fellow Franchise/Yarahuan/Protected/ValidationReview.csv", row.names = FALSE)
```
